...
| DOC.1 | Model the config sections the typed config does not. Dependencies, `tools.deny` names, the `alpi` command surface and every field of the typed config dataclasses are now derived from the code and tested against [CONFIG.md](CONFIG.md). What no test can reach is the rest of the reference: `alp`, `host`, `budget`, `network` and `relay` are `dict[str, Any]` with no declaration site, and two documented keys (`tools.budget.per_result_chars`, `tools.web_search.max_per_turn`) are read straight off `cfg.raw`, past the dataclass. Give those sections the same typed shape the others have and the reference pins itself; until then those rows are maintained by hand and nothing compares them. | 🔵 |
| PROC.2 | A run the watchdog kills should take its cause with it. The foreground `terminal` timeout no longer leaks: the command gets its own process group and the timeout signals the group, so a shell that backgrounded work does not leave it running. What remains is the sweep, which signals the run's own pid while MCP servers, background terminal jobs and pipeline gate processes each start their own session — the wrapper chain that caused the wedge survives the kill. Needs the run to track what it spawned; best folded into MCP.2. | 🔵 |
