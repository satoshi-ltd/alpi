...
tool of the `github` MCP server. What one peer's inbound `link.ask` turns
may run on this profile is set the other way round, as the only tools
allowed, under `tools.allow` in its `peers.yaml` record; see
...
token stored as a SHA-256 digest and its access preserved, and the source is
deleted once the destination has been re-read and verified. No backup copy is
written. The old schema has no grouping key, so migration cannot safely merge
rows that may belong to the same person or workload. Copies left by earlier
releases stay untouched; ``alpi doctor`` lists them.
