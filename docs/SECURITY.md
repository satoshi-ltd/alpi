...
- **Host plane two-layer trust** (v0.5). The control-plane API
  ...
  - **Unix socket** (`~/.alpi/host/host.sock`, mode 0600) — local
    ...
    machine.
  - **WebSocket on Tailscale or LAN** (default port 49200). Bind
    auto-detected to a CGNAT (`100.64.0.0/10`) or RFC1918 private
    address; the listener refuses `0.0.0.0`/loopback/public IPs
    up-front (`alpi/host/server.py::_validate_tcp_bind`). Every
    ...
    `auth-failed` (-32000). Until the first device is added the
    listener stays open (migration window for v0.4 setups).
