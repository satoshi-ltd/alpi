...
- `~/.alpi/host/connections.yaml` groups a connection identity (`label`, `role`, `profile_scope`) with independently revocable device credentials. Dispatcher resolves each token to connection/device IDs and gates scoped members on `params.profile in connection.profile_scope`, else `-32001 forbidden`. Sessions and daily ledger usage are attributed to the connection; local Unix/TUI/CLI activity uses synthetic `host`. `host.connections.*` is canonical and `host.devices.*` remains a compatibility alias. See `security` for the scope-free allowlist and migration behavior.
