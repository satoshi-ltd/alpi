...
- `knowledge(action="lint", path?)` / `knowledge(action="index", path?, force?)` — validate the bundle or rebuild the `okf_*` derived tables in `knowledge.sqlite`; indexing without a `path` also repairs the workspace bundle (required files, unlinked pages); a root given as an explicit `path` is never written to.
