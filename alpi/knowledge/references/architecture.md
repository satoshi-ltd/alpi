...
- Model iterations per turn are capped at `tools.max_steps_per_turn` (default 100) for every provider and budget. It bounds loops, refusals and the TODO guard, not cost. Hitting the cap does not discard work: normal chats get one tools-off best-effort wrap-up, while detached workgroup turns get one `workgroup_post`-only handoff.
- Host chat streams `usage` frames with accounting fields plus `context_tokens`. Only main-turn completions set `context_tokens`; side calls leave it at zero, so reconnect-capable clients can update the live context meter without confusing vision/research usage for conversation growth.

## Scheduler

- Agent jobs run via `alpi chat --once --emit-events --no-save`.
- Script jobs (`no_agent: true`) run directly, `shell=False`.
- Each fire is capped at `job.timeout` seconds (default 900, max 3600) — a stuck-process backstop for unattended runs, not the cost guard (`budget.daily_usd` is) and not a hint that jobs must be short. Heavy jobs (deep research, multi-step publishing) raise it via `schedule(add|update, timeout=…)`. The scheduler hands the child a soft budget (`ALPI_TURN_BUDGET_S`, ~10% reserved) so the engine finalizes before the hard `subprocess` kill; the scheduled prompt states that budget in minutes, and the engine adds one `[engine] Time check` user note at 50% (time left only) and one at 80% (wrap up, do not start a step that may not complete), so the model can skip a long last step instead of being killed inside it (the soft deadline is only checked between steps); workgroup dispatch turns get the same notes, worded as "close the turn the way this task expects" so they never read as a request to post progress. Normal jobs return one tools-off reply; detached workgroup turns make one `workgroup_post`-only handoff. A run killed at the hard limit reports what it was doing — tool in flight and for how long, tool calls made, last message — in `schedule.failed`, the inbox row and the run ledger row, which also carries the `run_id` for every scheduled run.
...
- `host.sessions.delete` bulk-deletes chat sessions by id; open to members but bound to the calling connection (a session owned by another connection answers `not-found`, never `forbidden` or `session-busy`, so ownership is checked before activity), then, inside the worker thread, claims the session slot exactly like a chat turn (`chat.claim_idle_session`, shared with Cleanup) so a turn starting mid-delete is refused on one side or the other, takes the exclusive sessions lock, refuses `session-busy` when an open run journal names the session (`runs.running_session_ids`, the same inventory Cleanup uses — an unverifiable `runs/` answers `cannot-verify`), then removes `sessions/<id>.json` + `_events_<id>.jsonl`. Claim, lock and check live in the thread so cancelling the RPC cannot release the claim while the delete is still running. The verb stays in the administrative audit trail whatever the caller's role.
- `~/.alpi/host/connections.yaml` groups a connection identity (`label`, `role`, `profile_scope`) with independently revocable device credentials (stored as SHA-256 digests) and hashed one-time pairing grants. Grants expire after ten minutes and `host.connections.exchange_pairing` consumes one atomically before returning the permanent device token once. Terminal grant metadata is kept for seven days, capped at 50 rows per connection and omitted from connection-list responses. Dispatcher resolves each permanent token to connection/device IDs and gates scoped members on `params.profile in connection.profile_scope`, else `-32001 forbidden`. Sessions and daily ledger usage are attributed to the connection; local Unix/TUI/CLI activity uses synthetic `host`. `host.connections.*` is canonical and `host.devices.*` remains a management compatibility alias. See `security` for the bootstrap exception, scope-free allowlist and migration behavior.
