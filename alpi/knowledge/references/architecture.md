...
- `host.attachments.{stage,fetch}`: `stage` uploads a file in; `fetch` serves an agent-made image's bytes (base64) out by path, so remote clients render inline images. `fetch` reads are scoped to the profile's workspace/home/temp (see `security`).
