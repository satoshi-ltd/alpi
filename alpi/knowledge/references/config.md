...
| `tools.max_steps_per_turn` | Per-turn tool-call ceiling (default 100) — a runaway-loop backstop, **not** the cost guard (`budget.daily_usd` is). Hitting it triggers one tools-off wrap-up call so the turn still returns a best-effort reply instead of failing. Left at the default, a free/ollama model or a profile with a positive daily budget lifts the ceiling to 1000; an explicit value is always respected. |
