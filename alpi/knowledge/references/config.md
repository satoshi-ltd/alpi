...
- WebSocket limits? -> daemon-wide `ALPI_HOST_WS_MAX_CONNECTIONS`, `ALPI_HOST_WS_MAX_CONNECTIONS_PER_DEVICE`, `ALPI_HOST_WS_MAX_RPCS_PER_DEVICE`, `ALPI_HOST_WS_AUTH_TIMEOUT`, `ALPI_HOST_WS_AUTH_RECHECK`, `ALPI_HOST_WS_CLOSE_TIMEOUT`, and `ALPI_HOST_WS_REVOCATION_RETRY` override the safe defaults and require restart. Per-IP limits belong at the trusted public edge; Alpi deliberately has no pre-auth global handshake budget that an attacker could exhaust.
