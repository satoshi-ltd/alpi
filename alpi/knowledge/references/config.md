...
| `tools.max_steps_per_turn` | Per-turn model-iteration ceiling (default 100) — a runaway-loop backstop, **not** the cost guard (`budget.daily_usd` is). Hitting it triggers one bounded finalization call: normal chats return a tools-off best-effort reply, while detached workgroup turns may call only `workgroup_post`. The configured value is literal for every provider and budget. |
| `tools.web_search.max_per_turn` | Per-turn `web_search` ceiling (default 25) — a runaway-loop backstop, not search discipline (the "cap at 3 per question" rule lives in the tool description). Searches are serialized and spaced ~1.5s apart regardless: overlapping calls are what tips a shared IP into a rate limit that lasts ~17 minutes. |
| `tools.web_search.backends` | Ordered `ddgs` text engines `web_search` asks one at a time, stopping at the first answer (default `duckduckgo, yahoo, brave` — provisional, from the one measurement available on a residential IP, not a claim the other `ddgs` engines are broken; engine behaviour depends on the host's IP, so a host may list others). Unknown names are dropped before they reach `ddgs`, which would otherwise widen them to `auto`; an empty or unusable list is the default. A search stops after 25 s: each engine runs in its own thread and is abandoned at the deadline, because `ddgs` joins its workers on exit. At most two engine requests are in flight: an engine whose abandoned request is still running is not asked again. An engine that times out, is rate-limited or errors cools down for 15 min, in memory only (a new process starts clean); an empty answer never does. When all are cooling, only the one due back first is asked. All-empty is reported as no results (`ok`), anything else without results as a failure; both name each engine's outcome, which is also logged at INFO. |
...
| `retention.{runs_days,sessions_days}` | Non-negative YAML integer days before the daily per-profile sweep; default `0` keeps forever. Invalid values, including booleans, fractions and quoted numbers, disable the window. Never deletes `logs/runs.jsonl`, active runs, busy sessions, or sessions of an existing `(wg_id=…)` workgroup. Session age and open runs are rechecked under the file lock shared with writers; an unverifiable run inventory stops session deletion and reports an error. Unknown workgroup identity is kept. Session cost is archived first. `alpi setup → Cleanup` remains manual with its own thresholds. |
...
| `alp.max_active_workgroups` | How many workgroups may be active at once (open task or running pipeline; `service._active_workgroup_ids`). Enforced only at pipeline admission, so it is a threshold and not a hard cap: deliberation launches and hub `#task` posts always open and merely count against it, and a workgroup that re-enters the active set another way (resume after a pause, a rewind, a `#task` re-opening a pipeline that closed `#done BLOCKED`) is never re-checked, so the active count can exceed the number. `pipeline_queue.limit_origin(home)` resolves the hub's explicit key → the default profile's key (daemon-wide, seeded at `5` by `seed_config_for`) → built-in unlimited; `0` = unlimited. `DEFAULT_CONFIG` carries no value for this key, so `config.save()` never materialises an override. Editable via `alpi workgroup limit N` (`--inherit` removes a hub override), `host.config.set_field` (integer ≥ 0, negatives rejected) and any profile's desktop settings; `host.profile.detail` exposes `max_active_workgroups`, `max_active_workgroups_origin` and `queued_pipelines`. |
...
- WebSocket limits? -> daemon-wide `ALPI_HOST_WS_MAX_CONNECTIONS`, `ALPI_HOST_WS_MAX_CONNECTIONS_PER_DEVICE`, `ALPI_HOST_WS_MAX_RPCS_PER_DEVICE`, `ALPI_HOST_WS_AUTH_TIMEOUT`, `ALPI_HOST_WS_AUTH_RECHECK`, `ALPI_HOST_WS_CLOSE_TIMEOUT`, and `ALPI_HOST_WS_REVOCATION_RETRY` override the safe defaults and require restart. `host.token_ttl_days` (absent = no expiry) expires a device token after that many days without use, judged on read against `last_seen`; expired devices stop authenticating and lose their live sockets until they pair again. `ALPI_HOST_WS_AUTH_FAILURES_PER_MINUTE` (default 10) caps authentication failures per source address: over budget, new sockets from that source close with 1013 before any token is read. `X-Forwarded-For` is trusted only from proxies listed in `ALPI_HOST_WS_TRUSTED_PROXIES` (IPs or CIDRs, empty by default); the client is the rightmost hop not in the list. The WSS overlay pins Caddy to `172.30.250.10` and lists it. There is no global handshake-per-minute budget; the global concurrent-socket cap is checked pre-auth and an anonymous socket occupies a slot until the auth timeout.
