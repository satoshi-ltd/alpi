  ...
  async function save({ kind, value }) {
    try {
      if (value == null) {
        ...
        await invoke("unset_config_field", { profile: profile.name, key: "budget.daily_tokens" });
        await invoke("set_config_field", { profile: profile.name, key: "budget.daily_usd", value: String(value) });
      } else {
        await invoke("unset_config_field", { profile: profile.name, key: "budget.daily_usd" });
        await invoke("set_config_field", { profile: profile.name, key: "budget.daily_tokens", value: String(value) });
      }
      await onSaved?.();
    } catch (e) {
