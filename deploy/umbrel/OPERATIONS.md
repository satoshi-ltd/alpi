...
- Alpi `0.4.2` -> Umbrel image tag `0.4.2`
- Alpi `0.4.2` -> `umbrel-app.yml` version `0.4.2`
...
docker buildx build \
  ...
  -t satoshiltd/alpi-umbrel:0.4.2 \
...
docker run --rm -it -p 8080:8080 -v alpi-umbrel-data:/data satoshiltd/alpi-umbrel:0.4.2
```
...
```bash
docker buildx imagetools inspect satoshiltd/alpi-umbrel:0.4.2
```

Copy the manifest list digest and use:

```yaml
image: satoshiltd/alpi-umbrel:0.4.2@sha256:<digest>
```
