...
- [ ] Build and publish `satoshiltd/alpi-umbrel:0.4.8` for
...
1. Add repository secrets:
   - `DOCKERHUB_USERNAME`
   - `DOCKERHUB_TOKEN`
2. Run the `Umbrel image` workflow manually with tag `0.4.8`.
...
- In `alpi/docker-compose.yml`, pin the image by digest:
  `satoshiltd/alpi-umbrel:0.4.8@sha256:<digest>`
...
For example, Alpi `0.4.8` ships as Umbrel image tag `0.4.8` and
