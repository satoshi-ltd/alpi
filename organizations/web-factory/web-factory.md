...
is the template (`templates/hotel-web/`, the 4-theme Astro kit) plus the
machine-readable contract (`factory/template-spec.json`). Agents produce
...
> alpi's own surfaces (`alpi/`, `desktop/`, `mobile/`). The `templates/hotel-web/`
> kit is the factory's PRODUCT, an idiomatic Astro app where `.ts` is
...
`new-project.py <slug> --brief <file>` clones the template, copies the brief,
resets `site.json` to a neutral skeleton, scaffolds the project, and creates
the `proj-<slug>` workgroup (hub **mira**, members scout/quill/lingua/muse/
pixel/lens — one per phase; budget from `org.yaml`
`budgets.project_workgroup`) with the pipeline:
...
- **template** (hub forge) — evolves `templates/hotel-web/`. Lessons from
  ≥3 live projects land in the template; each significant change ships an ADR
  under `templates/hotel-web/decisions/`.
- **quality** (hub vera) — owns the single pre-launch checklist (WCAG AA,
  ...
  `templates/hotel-web/quality/checklist.md`.
...
Builds the 11 profiles, the 3 standing workgroups, syncs the template + the
`factory/` contract into the workspace (`~/git/web-factory`), and scaffolds
`templates/hotel-web`, `projects/`, `archive/`.

## Spinning up a hotel

```bash
uv run python organizations/web-factory/new-project.py casa-bahia \
  --brief organizations/web-factory/briefings/boutique/brief.md
```

Then drive it through the pipeline. There are two fixture modes:

Every brief lives in `briefings/<name>/brief.md`, with any hotel-supplied
photos beside it in `briefings/<name>/assets/` (real-client photos are
gitignored via a local `.gitignore`; tiny fixtures are tracked).
...
```bash
uv run python organizations/web-factory/acceptance.py golden   # trivial-close path (no muse)
uv run python organizations/web-factory/acceptance.py visual   # assets path (muse before content)
uv run python organizations/web-factory/acceptance.py restore  # hotel-supplied photos restored into inventory slots
```
...
```bash
uv run python organizations/web-factory/new-project.py casa-bahia-golden \
  --brief organizations/web-factory/briefings/golden/brief.md
```

To tear a project down:

```bash
uv run python organizations/web-factory/new-project.py casa-bahia --nuke
```
