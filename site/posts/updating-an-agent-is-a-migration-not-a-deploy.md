...
- **Tool contract drift** creates the silent failure case. MCP servers serve their tool schemas at runtime with no committed artifact to diff in CI, so the fingerprint or the description can change with no review, no baseline, no gate. An agent that's been reliably calling Stripe or Slack just starts calling it wrong, and nothing fails loudly.
- **Memory and tools interact badly.** Under MEMDRIFT, a research dataset, long-term memory biases tool calls in contexts where it shouldn't — across 288 verified MCP servers it scanned 6,062 tools and flagged 608 with vulnerable parameters; biased memories raised tool-drift deflection by up to 3.6 points on a 1–5 scale. Memory compaction tends to make it worse, not better. Every update compounds the drift.
...
1. **Offline eval** on a fixed set — unit-level, tool-del-frame, trajectory-level.
...
3. **Canary** — route a small slice (commonly 5%+) of real traffic behind automated gates: error rate, latency, tool-call pattern, an eval score. Escalate weight on pass, revert on fail.
...
- **Separate run state from long-term memory.** Run state is mutable, per-execution. Long-term memory is an append-only, versioned history — keep the raw history immutable and write derived summaries separately, so you always have a source of truth to rebuild from. Compacting memory in place is how drift enters.
...
- **Treat tool interfaces as a versioned contract.** Commit the schema baseline, diff it in CI, alias old tool names. API teams learned this over a decade with REST and gRPC: never change the meaning of a field without versioning the contract.
- **Make rollback logical from day one.** Route reverts to the stable version without a redeploy, and design compensating actions for anything an agent might do externally.

## The knowledge is the asset, the runtime is replaceable

This is a discipline problem, but it's also a product-surface one. If your agent, its memory, its skills live inside a runtime that resists being diffed, versioned, and swapped, you cannot perform the migration no matter how disciplined you are. The cheap trick: keep what the agent knows in files you can review and revert, and keep the runtime replaceable.

Alpi treats agents as sovereign long-lived peers, not scripts. Skills and operator knowledge live as plain Markdown in your own files — so an update is a reviewable diff, and the accumulated knowledge survives the process that runs it. When the runtime changes and you need a clean starting state, you don't lose the memory someone spent weeks building. You don't recover a balloon payment in pain later.

You don't need to relearn the moral: agents fall into the pit of being refreshed by versions, but the same data discipline. Put versioned knowledge files now. The rework is the price you pay for a working evaluation system. `uv tool install alpi-agent`