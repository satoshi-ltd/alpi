...
happens via `alf setup`, never inline. If nothing is configured we point
the user there.
"""
...
from alf.tui.screens import FloatingPanel
...
class ProviderPanel(FloatingPanel):
    ...
    def compose_body(self) -> ComposeResult:
        ...
        if not options:
            yield Static(
                "No providers configured yet. Run `alf setup` from the "
    ...
    def _build_options(self) -> list[Option]:
        ...
        for p in prov_mod.builtin():
            ...
            # least one model id via `alf setup` — we don't fetch the
...
class ModelListPanel(FloatingPanel):
    ...
    def on_option_list_option_selected(self, event: OptionList.OptionSelected) -> None:
        ...
        # to config.yaml. The saved default only changes via `alf setup`.
        ...
        from alf.tui.widgets import DimLine
