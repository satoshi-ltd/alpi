"""Per-skill env scoping (task AV) — close the residual exfiltration
vector left by the v0.2.85 file-tool denylist.

A prompt-injected skill must NOT see secrets via ``terminal()``
subprocess. The terminal tool default-denies ``os.environ`` down to
a small safelist; a skill opts back into specific vars by declaring
``env: [FOO_API_KEY]`` in its frontmatter.
"""
