...
def fake_engine(monkeypatch):
    ...
    class _FakeEngine:
        ...
        def save_session(self):
            path = self.home / "sessions" / f"{self.session.id}.json"
...
def test_fresh_chat_creates_and_binds_new_session(tmp_path: Path, fake_engine) -> None:
    ...
    assert (tmp_path / "sessions" / f"{bound}.json").exists()
...
def test_second_inbound_same_chat_resumes_same_session(
    ...
    data = json.loads((tmp_path / "sessions" / f"{first_id}.json").read_text())
...
def test_different_chats_stay_isolated(tmp_path: Path, fake_engine) -> None:
    ...
    work_data = json.loads((tmp_path / "sessions" / f"{work}.json").read_text())
    personal_data = json.loads((tmp_path / "sessions" / f"{personal}.json").read_text())
...
def test_forget_gets_a_fresh_session_next_inbound(
    ...
    assert old != new
    # Old session file untouched.
    assert (tmp_path / "sessions" / f"{old}.json").exists()
    assert (tmp_path / "sessions" / f"{new}.json").exists()
