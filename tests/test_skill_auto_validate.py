"""Every mutating action on a user skill runs the script validator and
surfaces findings in the tool output. The LLM can iterate in the same
turn without calling `validate` separately."""
...
def test_edit_body_revalidates(isolated_home: Path) -> None:
    ...
    # Edit the SKILL.md to reference a wrong port — port-coherence should fire.
    r = Skill().run(
