...
def test_pipe_to_interpreter_variants_are_dangerous(cmd: str) -> None:
    assert classify(cmd)[0] == Severity.DANGEROUS, cmd
...
def test_benign_pipes_are_not_dangerous(cmd: str) -> None:
    sev, _ = classify(cmd)
    assert sev != Severity.DANGEROUS, f"{cmd} → {sev}"
